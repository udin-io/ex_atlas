defmodule Atlas do
  @moduledoc """
  Atlas is a composable, pluggable Elixir SDK for managing GPU and CPU compute
  across multiple cloud providers (RunPod, Fly.io Machines, Lambda Labs, Vast.ai,
  or any module you write that implements `Atlas.Provider`).
  ...
  ## Running a serverless inference job
      ...
      {:ok, done} = Atlas.get_job(job.id)
      done.output

  ## Stream a job

      Atlas.stream_job(job.id) |> Enum.each(&IO.inspect/1)

  ## Swapping providers

      Atlas.spawn_compute(provider: :runpod, gpu: :h100, ...)
      Atlas.spawn_compute(provider: :lambda_labs, gpu: :h100, ...)  # v0.2
      Atlas.spawn_compute(provider: MyInternalCloud.Provider, gpu: :h100, ...)

  See `Atlas.Provider` for the behaviour contract and `Atlas.Config` for how
  provider + API key resolution works.
  """

  alias Atlas.{Config, Spec}
  ...
  `Atlas.Spec.ComputeRequest`. See `Atlas.Spec.ComputeRequest` for the full
  field list.
  """
