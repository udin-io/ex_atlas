defmodule ExAtlas.Providers.Mock do
  @moduledoc """
  In-memory provider for tests and demos.

  Every `ExAtlas.Provider` callback is implemented against an ETS-backed store.
  Useful for:
  ...
  def get_compute(id, _ctx) do
    ...
    case :ets.lookup(@table, {:compute, id}) do
      ...
      [] -> {:error, ExAtlas.Error.new(:not_found, provider: :mock, message: "no compute #{id}")}
    end
  end
