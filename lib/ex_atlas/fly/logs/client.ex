defmodule ExAtlas.Fly.Logs.Client do
  @moduledoc """
  ...
  alias ExAtlas.Fly.Logs.LogEntry
  ...
  ## Options
    ...
    * `:base_url` — override the endpoint (default: `:ex_atlas, :fly, log_endpoint` or
      `#{@default_base_url}`).
  ...
  ## Options
    ...
    * `:token_fn` — `(app_name -> {:ok, token} | {:error, reason})`. Defaults to
      `ExAtlas.Fly.Tokens.get/1`.
    * `:invalidate_fn` — `(app_name -> :ok)`. Defaults to `ExAtlas.Fly.Tokens.invalidate/1`.
  ...
  defp base_url(opts) do
    opts[:base_url] ||
      Application.get_env(:ex_atlas, :fly, [])[:log_endpoint] ||
      @default_base_url
  end
