if Code.ensure_loaded?(Phoenix.LiveDashboard.PageBuilder) do
  defmodule ExAtlas.LiveDashboard.ComputePage do
    @moduledoc """
    ...
    Add `:phoenix_live_dashboard` and `:ex_atlas` to your Phoenix app's deps, then
    ...
    subscribes to `ExAtlas.PubSub` when the socket connects.
    """
    ...
    @impl true
    def init(_opts), do: {:ok, %{}, application: :ex_atlas}

    @impl true
    def menu_link(_session, _capabilities), do: {:ok, "ExAtlas"}

    @impl true
    def mount(_params, _session, socket) do
      if connected?(socket) and pubsub_available?() do
        Phoenix.PubSub.subscribe(ExAtlas.PubSub, "atlas:compute")
      end
    ...
    defp pubsub_available? do
      Code.ensure_loaded?(Phoenix.PubSub) and Process.whereis(ExAtlas.PubSub) != nil
    end
  end
end
