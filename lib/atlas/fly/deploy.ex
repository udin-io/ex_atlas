defmodule Atlas.Fly.Deploy do
  ...
  through `Atlas.Fly.Dispatcher`, with a 5 min activity timeout and a 30 min
  absolute cap.

  Streamed output lands as `{:atlas_fly_deploy, ticket_id, line}` on the topic
  `"atlas_fly_deploy:\#{ticket_id}"`.
  """
  ...
  def deploy(project_path, fly_toml_dir) do
    ...
    unless File.dir?(deploy_dir) do
      ...
      Logger.info("[Atlas.Fly.Deploy] Running `fly deploy` in #{deploy_dir}")
      ...
      case Task.yield(task, @fly_deploy_timeout_ms) || Task.shutdown(task, :brutal_kill) do
        {:ok, {output, 0}} ->
          Logger.info("[Atlas.Fly.Deploy] Deploy succeeded")
          {:ok, output}

        {:ok, {output, exit_code}} ->
          Logger.error("[Atlas.Fly.Deploy] Deploy failed (exit #{exit_code}): #{output}")
          {:error, {:fly_error, exit_code, output}}

        nil ->
          Logger.error("[Atlas.Fly.Deploy] Deploy timed out after #{@fly_deploy_timeout_ms}ms")
  ...
  @doc """
  Run `fly deploy --remote-only` and stream output via `Atlas.Fly.Dispatcher`.

  For each non-empty line, broadcasts `{:atlas_fly_deploy, ticket_id, line}`
  on `"atlas_fly_deploy:\#{ticket_id}"`.
  ...
  def stream_deploy(project_path, fly_toml_dir, ticket_id) do
    ...
    unless File.dir?(deploy_dir) do
      ...
      Logger.debug("[Atlas.Fly.Deploy] Streaming `fly deploy` in #{deploy_dir}")

      case System.find_executable("fly") do
        nil ->
          Logger.error("[Atlas.Fly.Deploy] `fly` executable not found in PATH")
  ...
  defp collect_port_output(port, ticket_id, acc, activity_ref, absolute_ref) do
    receive do
      ...
      {^port, {:exit_status, 0}} ->
        ...
        Logger.debug("[Atlas.Fly.Deploy] Streaming deploy succeeded")
        {{:ok, output}, activity_ref}

      {^port, {:exit_status, exit_code}} ->
        output = acc |> Enum.reverse() |> IO.iodata_to_binary()
        Logger.error("[Atlas.Fly.Deploy] Streaming deploy failed (exit #{exit_code})")
      ...
      {:deploy_activity_timeout, ^port} ->
        ...
        Logger.error(
          "[Atlas.Fly.Deploy] Streaming deploy stalled (no output for #{@fly_activity_timeout_ms}ms)"
        )
      ...
      {:deploy_absolute_timeout, ^port} ->
        ...
        Logger.error(
          "[Atlas.Fly.Deploy] Streaming deploy hit absolute timeout (#{@fly_max_timeout_ms}ms)"
        )
