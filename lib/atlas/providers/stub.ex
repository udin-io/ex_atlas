defmodule Atlas.Providers.Stub do
  @moduledoc """
  Shared base for placeholder providers that haven't been implemented yet.

  Future `Atlas.Providers.Fly`, `Atlas.Providers.LambdaLabs`, `Atlas.Providers.Vast`
  modules `use` this to reserve the provider atom in `Atlas.Config`, expose an
  ...
  defmacro __using__(opts) do
    ...
    quote bind_quoted: [provider: provider, caps: caps, docs_url: docs_url] do
      ...
      def stream_job(_id, _ctx),
        do: Stream.map([{:error, Atlas.Error.new(:unsupported, provider: @provider_atom)}], & &1)
      ...
      defp not_implemented(fun) do
        msg =
          "Atlas provider #{inspect(@provider_atom)}.#{fun}/_ is not implemented yet." <>
            if(@docs_url, do: " See #{@docs_url} for status.", else: "")

        {:error, Atlas.Error.new(:unsupported, provider: @provider_atom, message: msg)}
      end
    end
  end
end
