defmodule Atlas.Providers.RunPod.Client do
  @moduledoc """
  ...
  win.
  """
  @spec management(Atlas.Provider.ctx()) :: Req.Request.t()
  ...
  """
  @spec runtime(Atlas.Provider.ctx(), String.t()) :: Req.Request.t()
  ...
  """
  @spec graphql(Atlas.Provider.ctx()) :: Req.Request.t()
  def graphql(ctx) do
    api_key = fetch_key!(ctx)
  ...
  @doc """
  Normalize a Req result into `{:ok, body} | {:error, Atlas.Error.t()}`.
  ...
  def handle_response({:error, other}, _expected) do
    {:error, Atlas.Error.new(:transport, provider: :runpod, raw: other)}
  end
    ...
    raise Atlas.Error,
      ...
      message:
        "no RunPod API key configured. Pass `api_key:` per call, set " <>
          "`config :atlas, :runpod, api_key: \"...\"`, or set RUNPOD_API_KEY."
  end
