defmodule Atlas.Orchestrator.Reaper do
  @moduledoc """
  ...
  On each tick, the Reaper:
    ...
    3. Flags any resource that exists at the provider but has no local tracker
       ...
       `Atlas.terminate/2` to reclaim the runaway spend.
  ...
  alias Atlas.Orchestrator.ComputeRegistry
  ...
  @impl true
  def init(_opts) do
    cfg = Application.get_env(:atlas, :orchestrator, [])
  ...
  defp reap_provider(provider, prefix) do
    case Atlas.list_compute(provider: provider, status: :running) do
      {:ok, computes} ->
        ...
        |> Enum.each(fn compute ->
          _ = Atlas.terminate(compute.id, provider: provider)
        end)
