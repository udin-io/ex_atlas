defmodule Atlas.Orchestrator.ComputeServer do
  @moduledoc """
  One `GenServer` per tracked compute resource.

  Responsibilities:

    * Hold the resource's normalized `Atlas.Spec.Compute`, its `:user_id`,
      `:idle_ttl_ms`, last-activity timestamp, and spawn opts.
    * Trap exits so `terminate/2` always calls `Atlas.terminate/2` on the
  ...
  alias Atlas.Orchestrator.{ComputeRegistry, Events}

  @default_idle_ttl_ms 30 * 60 * 1_000
  @default_heartbeat_interval_ms 60 * 1_000

  @type state :: %{
          compute: Atlas.Spec.Compute.t(),
          opts: keyword(),
  ...
  def terminate(reason, state) do
    ...
    case Atlas.terminate(state.compute.id, state.opts) do
      :ok ->
