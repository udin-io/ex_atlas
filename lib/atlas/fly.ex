defmodule Atlas.Fly do
  @moduledoc """
  Fly.io platform operations for Atlas.

  This is the public facade for everything Fly-related in atlas. It deliberately
  covers *platform* operations (deploys, logs, token lifecycle) — for GPU
  compute, see `Atlas.spawn_compute/2` and the provider behaviour.

  ## Quick start

      # In config/config.exs (optional — defaults are sensible)
      config :atlas, :fly,
        enabled: true,
        dispatcher: :registry,
        storage_path: "priv/atlas_fly"

  ### Discover apps from `fly.toml`

      Atlas.Fly.discover_apps("/path/to/project")
      # => [{"my-api", "/path/to/project"}, {"my-web", "/path/to/project/web"}]

  ### Tail logs

      Atlas.Fly.subscribe_logs("my-api", "/path/to/project")

      # Then in your GenServer / LiveView:
      def handle_info({:atlas_fly_logs, "my-api", entries}, state) do
        # entries is a list of Atlas.Fly.Logs.LogEntry
        ...
      end

  ### Deploy with streaming output

      Atlas.Fly.subscribe_deploy(ticket_id)
      Atlas.Fly.stream_deploy(project_path, "web", ticket_id)

      def handle_info({:atlas_fly_deploy, ^ticket_id, line}, state) do
        ...
      end
  """

  alias Atlas.Fly.Dispatcher
  alias Atlas.Fly.Logs.{Streamer, StreamerSupervisor}

  @doc "See `Atlas.Fly.Deploy.discover_apps/1`."
  defdelegate discover_apps(project_path), to: Atlas.Fly.Deploy

  @doc "See `Atlas.Fly.Deploy.deploy/2`."
  defdelegate deploy(project_path, fly_toml_dir), to: Atlas.Fly.Deploy

  @doc "See `Atlas.Fly.Deploy.stream_deploy/3`."
  defdelegate stream_deploy(project_path, fly_toml_dir, ticket_id), to: Atlas.Fly.Deploy

  @doc """
  Subscribe the calling pid to log events for `app_name`, starting a streamer
  if none is running.
  """
  @spec subscribe_logs(String.t(), String.t(), keyword()) :: :ok
  def subscribe_logs(app_name, project_dir, opts \\ []) do
    opts =
      Keyword.merge(opts,
        registry: StreamerSupervisor.registry_name(),
        dynamic_sup: StreamerSupervisor.dynamic_supervisor_name()
      )

    Streamer.subscribe(app_name, project_dir, opts)
  end

  @doc "Unsubscribe the calling pid from log events for `app_name`."
  @spec unsubscribe_logs(String.t()) :: :ok
  def unsubscribe_logs(app_name) do
    Dispatcher.unsubscribe("atlas_fly_logs:#{app_name}")
  end

  @doc "Subscribe the calling pid to streamed deploy output for `ticket_id`."
  @spec subscribe_deploy(String.t()) :: :ok | {:error, term()}
  def subscribe_deploy(ticket_id) do
    Dispatcher.subscribe("atlas_fly_deploy:#{ticket_id}")
  end

  @doc "Unsubscribe the calling pid from streamed deploy output for `ticket_id`."
  @spec unsubscribe_deploy(String.t()) :: :ok
  def unsubscribe_deploy(ticket_id) do
    Dispatcher.unsubscribe("atlas_fly_deploy:#{ticket_id}")
  end

  @doc "See `Atlas.Fly.Logs.StreamerSupervisor.stop_streamer/1`."
  defdelegate stop_streamer(app_name), to: StreamerSupervisor

  @doc "See `Atlas.Fly.Logs.StreamerSupervisor.streamer_running?/1`."
  defdelegate streamer_running?(app_name), to: StreamerSupervisor
end
