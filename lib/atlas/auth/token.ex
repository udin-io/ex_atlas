defmodule Atlas.Auth.Token do
  @moduledoc """
  Bearer-token authentication for the "transient per-user pod" pattern.

  When you spawn a compute resource with `auth: :bearer`, Atlas:
  ...
  The raw token is never stored by Atlas. Treat it like a password: either hand
  ...
  ## Typical flow
      ...
      {:ok, compute} = Atlas.spawn_compute(gpu: :h100, auth: :bearer, ports: [{8000, :http}])
