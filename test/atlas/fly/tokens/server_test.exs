defmodule Atlas.Fly.Tokens.ServerTest do
  use ExUnit.Case, async: false

  alias Atlas.Fly.Tokens.Server
  alias Atlas.Fly.TokenStorage.Memory
  ...
  setup do
    ...
    test_name = :"atlas_fly_tokens_#{System.unique_integer([:positive])}"
  ...
  describe "config file resolution" do
    ...
    test "skips config file when fly_config_file_enabled is false", context do
      ...
      previous = Application.get_env(:atlas, :fly, [])

      try do
        Application.put_env(:atlas, :fly, Keyword.put(previous, :fly_config_file_enabled, false))
        start_server(context, config_file_fn: config_file_fn, cmd_fn: cmd_fn)

        assert {:error, :no_token_available} = Server.get_token(@app_name, context.test_name)
      after
        Application.put_env(:atlas, :fly, previous)
      end
    end
  end
